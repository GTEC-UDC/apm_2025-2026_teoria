...
- *1 de cada 6 personas* en el mundo tiene algún tipo de diversidad funcional (OMS)

- Diversidad funcional incluye: visual, auditiva, motriz, cognitiva --- permanente, temporal o situacional

- *Ejemplos situacionales:*
  - Llevar gafas de sol → contraste reducido (visual temporal)
  - Brazo en cabestrillo → una sola mano (motriz temporal)
  - Entorno ruidoso → no se oye el audio (auditiva situacional)
...
#block(
  ...
  La accesibilidad no es un _"nice to have"_ --- es un requisito de calidad y, en muchos contextos, una *obligación legal*.
]
...
#grid(
  ...
  Android incluye herramientas de accesibilidad en *Ajustes → Accesibilidad*:

  - *Lectores de pantalla*: TalkBack (navegación completa por voz), Select to Speak (lectura bajo demanda)
  - *Pantalla*: tamaño de fuente/pantalla, magnificación, corrección e inversión de color.
  - *Interacción*: Switch Access (botones físicos en lugar de pantalla táctil), tiempos de espera.
  - *Audio y texto*: subtítulos automáticos, transcripción en tiempo real, amplificación de audio.
  ...
  #block(
    ...
    Algunas funcionan automáticamente con buenas prácticas; otras requieren las *APIs de accesibilidad*.
  ]
][
  #align(center, image("images/accessibility_settings.png", width: 100%))
]
...
#grid(
  ...
  Lector de pantalla integrado en Android, pensado para usuarios ciegos o con baja visión.

  Con TalkBack activo cambia el modo de interacción:

  - *Explorar tocando*: tocar o arrastrar el dedo por la pantalla. TalkBack lee en voz alta el elemento bajo el dedo
  - *Navegación secuencial*: swipe rápido izquierda/derecha. Mueve el foco al elemento anterior/siguiente según el orden de la UI.
  - *Doble tap*: activa el elemento enfocado
  // - *Swipe abajo → derecha*: abre el menú de acciones personalizadas.
  ...
  #block(
    ...
    *Para el desarrollador*: cada elemento debe tener una descripción que TalkBack pueda leer. Sin ella, TalkBack solo puede anunciar el tipo de componente.
  ]
][
  #align(center, image("images/talkback_navigation.png", width: 100%))
]
...
#grid(
  ...
  Permite interactuar mediante uno o más botones físicos externos en lugar de tocar la pantalla. Diseñado para usuarios con *movilidad reducida*.

  Mecanismo de navegación:

  - Un botón navega por los elementos secuencialmente (resaltándolos uno a uno).
  - Un segundo botón activa el elemento resaltado.

  - Para testing, se pueden emplear los botones de volumen.
][
  #image("images/switch_navigation.png", width: 100%)
]


== El modelo de accesibilidad en Android

#grid(
  columns: (1.8fr, 1fr),
  column-gutter: 2em,
)[
  El desarrollador escribe una sola app. Android se encarga de comunicar la información a cada servicio de accesibilidad (TalkBack, Switch Access, etc.).

  - *Componentes estándar* (botones, textos, switches...): funcionan automáticamente.

  - *Elementos no estándar* (imágenes, iconos, FABs...): el framework no puede inferir la descripción — el desarrollador debe usar las APIs de accesibilidad.
][
  #align(center, image("images/model_unlabeled_button2.png", height: 12em))

  #align(center, text(
    size: 0.78em,
    fill: luma(20%),
  )[Sin `contentDescription`, TalkBack solo puede decir _"Unlabeled Button"_])
]


== Touch targets y contraste

*Touch targets*

- Tamaño mínimo recomendado: 48 $times$ 48 dp. Es el valor por defecto en M3.
- El área táctil puede ser mayor que el elemento visual.
- Los elementos interactivos no deben estar demasiado próximos entre sí para evitar toques accidentales.

*Contraste de color (Web Content Accessibility Guidelines 2.1)*
...
#grid(
  ...
  - Texto grande (≥ 18 sp): ratio mínimo *3:1*
  - Elementos de UI y gráficos: mínimo *3:1*
  - *No usar solo color* para transmitir información (complementar con iconos, texto, patrones)
][
  #figure(
    image("images/m3-contrast.png", height: 6.5em),
    caption: text(size: 0.85em)[Contraste suficiente (izquierda) vs. contraste pobre (derecha).],
  )
]
...
- *Etiquetas*: todo elemento interactivo debe tener una descripción accesible mediante `contentDescription`, que describa el propósito, y el estado cuando sea relevante:
  #[
    #set text(size: 0.95em)
    ```kotlin
    // Describe propósito + estado
    Icon(Icons.Default.ShoppingCart, contentDescription = "Carrito de compra: 3 artículos")

    // Imagen decorativa: null para que los servicios de accesibilidad la ignoren
    Image(painter, contentDescription = null)
    ```
  ]
- *Orden de lectura*: estructurar la UI para que los servicios de accesibilidad la recorran en orden lógico.
- *Agrupación semántica*: `Modifier.semantics(mergeDescendants = true)` agrupa elementos relacionados (p.ej. título + subtítulo + imagen de una tarjeta) para que se anuncien como una unidad.
- *Encabezados*: `Modifier.semantics { heading() }` marca un elemento como título de sección, permitiendo saltar entre encabezados con un gesto.
...
- *Navegar con TalkBack y Switch Access* como lo haría un usuario real: verificar que todos los elementos se anuncian correctamente y son accionables.

#v(1.5em)

*Automatizado:*

- *Accessibility Scanner*: app de Google que analiza la pantalla y detecta problemas (touch targets, contraste, etiquetas).

- *Compose UI testing*: verificar etiquetas y semántica desde los tests:
  #v(0.75em, weak: true)
  #[
    ```kotlin
    composeTestRule.onNodeWithTag("cart")
        .assertContentDescriptionEquals("Carrito: 3 artículos")
    ```
  ]
...
#grid(
  ...
  *Herramientas y ejemplos*
  ...
  - #link("https://www.figma.com/community/plugin/734693888346349671")[Material Theme Builder (Figma)]
